...
print("=" * 50)
print("FEDERATED LEARNING EXPERIMENTS (Simplified)")
print("=" * 50)
...
def federated_average(global_model, client_models):
    ...
    for key in global_dict.keys():
        # Skip non-float parameters like running mean/variance in BatchNorm
        if global_dict[key].dtype == torch.float32 or global_dict[key].dtype == torch.float64:
            # Average the float parameters
        ...
        # For integer/long parameters, keep original (no averaging needed)
    
    global_model.load_state_dict(global_dict)
    return global_model

# First, train centralized baseline with this model
